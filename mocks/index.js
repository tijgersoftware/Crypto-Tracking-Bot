...
export async function fetchCurrencyRate(pair, use, factor) {
  ...
  if (response.status === 200) {
    ...
    if (use !== "initial") {
      ...
      const initialPrice = initialValueArray.find(
        (el) => el.pairs === pair
      ).ask;
...
export async function logAlert(
  ...
  const data = {
    ...
    updateAllPairs: config.updateAllPairs,
    pairsFile: config.pairs,
  };
  console.log(
    `Timestamp: ${data.timestamp}, Pair: ${data.pair}, Initial Value: ${
      data.initial_value
    }, Value Now: ${data.rate}, Percentage: ${percentage} boundary: ${
      factor * 100
    }%`
  );
}
...
export async function fetchInitialValues(pairs, fetchInterval) {
  ...
  for (let i = 0; i < pairs.length; i++) {
    ...
    initialValueArray.push({ pairs: pairs[i], ask: price });
  }
...
export async function updatePairsInitialValues() {
  ...
  const initialValueArray = await readInitialValues();
  const pairsNotInInitialValues = await checkInitialValues(
    pairs,
    initialValueArray
  );
  ...
  for (let i = 0; i < pairs.length; i++) {
    ...
    try {
      ...
      initialValueArray.push({ pairs: pair, ask: price });
    }
  }
...
export async function main() {
  ...
  for (let i = 0; i < pairs.length; i++) {
    ...
    if (config.refresh > 0) {
      [config, pairs, previousConfigTime] = await reloadConfig(
        config,
        pairs,
        previousConfigTime,
        config.refresh
